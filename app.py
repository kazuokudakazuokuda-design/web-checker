...
# 概要・免責事項の表示 [cite: 77, 78, 133, 134, 230, 231]
st.markdown("""
# 🛡️ Web構造比較診断
**【概要】** 本ツールは、自社と競合のWebサイトを「物理構造」と「コンテンツ内容」の両面から比較し、4つのステップで実務的な改善案を抽出します [cite: 77, 133, 230]。  
**【免責事項】** 本診断は生成AIによる診断です。推測、不正確な情報を含む可能性がありますので、一次診断用として参考に使用ください [cite: 78, 134, 231]。
""")
...
def analyze_site_physics(url, limit_pages=40):
    ...
    return {
        ...
        "body_preview": soup.get_text()[:1500] # 更新日抽出用に長めに取得
    }

# --- 3. メインUI ---
col_u1, col_u2, col_u3 = st.columns(3)
with col_u1: my_url = st.text_input("自社URL", key="my_url_in", placeholder="https://example.com") [cite: 79, 135, 232]
with col_u2: comp1_url = st.text_input("競合A", key="c1_url_in", placeholder="https://comp-a.com") [cite: 82, 139, 235]
with col_u3: comp2_url = st.text_input("競合B (任意)", key="c2_url_in", placeholder="https://comp-b.com") [cite: 83, 140, 237]

# STEP 1: 業界判定
if st.button("STEP 1：業界を判定"): [cite: 81, 137, 234]
...
if st.session_state.step >= 2:
    ...
    if st.button("診断レポートを一括生成"): [cite: 91, 147, 245]
        with st.spinner("詳細レポートを構築中..."):
            ...
            【STEP 3：診断レポート】
            以下の3つの見出し（##）で構成し、各小見出しの下は「3つのブロック（改行）」で記述せよ。
            ...
            見出しサイズはSTEP2、3と同じにせよ。
            1. サマリー
               - 1. 診断結果のまとめ：数値差が招いている最大の構造的弱点を整理。
               - 2. 提案の骨子：戦略核を文章で記述せよ。
            2. 優先度別提言（最優先/優先/次の課題）
               各項目に「（最初の一歩）」を含め、解析キーワードを引用した具体的タスクを提示せよ。
            3. 新コンテンツ３案
               タイトルと詳細な概要のみ。目的などの重複項目は削除。
