        ...
        with st.spinner("詳細レポートを構築中..."):
            ...
            3. 新コンテンツ３案
               ...
               1. 判別できる場合（自社・競合のデータに明確な強みがある時）
                  - アクション: 抽出されたデータに基づき、具体的なテーマ（例：相続、交通事故、外壁塗装など）で3つのコンテンツ案を即座に作成せよ。
                  - 内容: そのテーマにおいて、顧客が最も懸念するポイントを深掘りした構成にせよ。
               2. 判別できない場合（データが不足している時）
                  - アクション: 「判別できない」とは一切言わず、**「注力したいと考えている特定の分野について、以下のような切り口でコンテンツを作ることを推奨します」**という一文から開始せよ。
                  - 業界固有の急所（中身）: STEP 1で判定した業界の筋道に基づき、顧客が必ずチェックする**「業界固有の重要ポイント」**を軸にした詳細な構成案を提示せよ。
