...
def analyze_site_physics(url, limit_pages=50):
    try:
        ...
        # エラー時は辞書形式でエラー内容を返すように修正
        return {"error": True, "message": str(e), "desc": "解析エラー"}
    ...
    if not my_url or not comp1_url:
        ...
        with st.spinner("メタデータ解析中..."):
            ...
            if "error" in st.session_state.my_data or "error" in st.session_state.c1_data:
                st.error("一部のURL解析に失敗しました。URLが正しいか、またはサイトがアクセスを拒否していないか確認してください。")
            else:
                prompt_s1 = f"""
                以下のメタディスクリプションから、このサイトが属する「業界名」を【単語1つのみ】で回答してください。
                解説や比較、選択肢の提示は一切不要です。

                自社Meta: {st.session_state.my_data.get('desc', '未設定')}
                競合AMeta: {st.session_state.c1_data.get('desc', '未設定')}
                ...
                """
                try:
                    res = model.generate_content(prompt_s1)
                    st.session_state.industry = res.text.strip()
                    st.session_state.step = 2
                except Exception as e:
                    st.error(f"AI判定中にエラーが発生しました: {e}")
...
if st.session_state.step >= 2:
    ...
    st.write(f"・自社Meta: {st.session_state.my_data.get('desc', '未設定')}")
    st.write(f"・競合AMeta: {st.session_state.c1_data.get('desc', '未設定')}")
    if st.session_state.c2_data:
        st.write(f"・競合BMeta: {st.session_state.c2_data.get('desc', '未設定')}")

    # --- STEP 2：調査レポート ---
    if st.button("STEP 2：調査レポートを生成"):
        with st.spinner("比較レポート生成中..."):
            def get_stats(d):
                if not d or "error" in d: return "データ取得失敗"
                return f"title:{d['title']}, h1:{d['h1']}, h2:{d['h2']}, a:{d['total_links']}, sitemap:{d['sitemap']}, 内部リンク:{d['internal_links_count']}, 資産:{d['asset_counts']}"
            
            c1_info = f"【競合A】{get_stats(st.session_state.c1_data)} / 本文抜粋: {st.session_state.c1_data.get('body_text', '')}"
            c2_info = f"\n【競合B】{get_stats(st.session_state.c2_data)} / 本文抜粋: {st.session_state.c2_data.get('body_text', '')}" if st.session_state.c2_data else ""
            ...
            【自社】{get_stats(st.session_state.my_data)} / 本文抜粋: {st.session_state.my_data.get('body_text', '')}
            {c1_info}{c2_info}
            """
            try:
